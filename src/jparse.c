...
__section__(".rare")
void SI_willDecodeSublist(json_decoder* decoder, const char* name, json_value_type type)
{
    if (type == kJSONArray)
    {
        decoder->userdata = playdate->system->realloc(
            NULL, sizeof(JsonArray)
        );
...
__section__(".rare")
void SI_didDecodeArrayValue(json_decoder* decoder, int pos, json_value value)
{
    --pos; // one-indexed (!!)
...
__section__(".rare")
void SI_didDecodeTableValue(json_decoder* decoder, const char* key, json_value value)
{
...
__section__(".rare")
void* SI_didDecodeSublist(json_decoder* decoder, const char* name, json_value_type type)
{
    return decoder->userdata;
}

__section__(".rare")
void free_json_data(json_value v)
{
...
__section__(".rare")
int parse_json(const char* path, json_value* out)
{
    if (!out) return 0;
    out->type = kJSONNull;
